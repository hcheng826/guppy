...
abstract contract GToken is GTokenInterface, ExponentialNoError, TokenErrorReporter {
     ...
     * @param guptroller_ The address of the Guptroller
     ...
     */
    function initialize(GuptrollerInterface guptroller_,
        ...
        // Set the guptroller
        uint err = _setGuptroller(guptroller_);
        require(err == NO_ERROR, "setting guptroller failed");

        // Initialize block number and borrow index (block number mocks depend on guptroller being set)
    ...
    function transferTokens(address spender, address src, address dst, uint tokens) internal returns (uint) {
        ...
        uint allowed = guptroller.transferAllowed(address(this), src, dst, tokens);
        if (allowed != 0) {
            revert TransferGuptrollerRejection(allowed);
        }
        ...
        // unused function
        // guptroller.transferVerify(address(this), src, dst, tokens);
     ...
     * @dev This is used by guptroller to more efficiently perform liquidity checks.
    ...
    function mintFresh(address minter, uint mintAmount) internal {
        ...
        uint allowed = guptroller.mintAllowed(address(this), minter, mintAmount);
        if (allowed != 0) {
            revert MintGuptrollerRejection(allowed);
        }
        ...
        // unused function
        // guptroller.mintVerify(address(this), minter, actualMintAmount, mintTokens);
    }
    ...
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn) internal {
        ...
        uint allowed = guptroller.redeemAllowed(address(this), redeemer, redeemTokens);
        if (allowed != 0) {
            revert RedeemGuptrollerRejection(allowed);
        }
        ...
        /* We call the defense hook */
        guptroller.redeemVerify(address(this), redeemer, redeemAmount, redeemTokens);
    }
    ...
    function borrowFresh(address payable borrower, uint borrowAmount) internal {
        ...
        uint allowed = guptroller.borrowAllowed(address(this), borrower, borrowAmount);
        if (allowed != 0) {
            revert BorrowGuptrollerRejection(allowed);
        }
    ...
    function repayBorrowFresh(address payer, address borrower, uint repayAmount) internal returns (uint) {
        ...
        uint allowed = guptroller.repayBorrowAllowed(address(this), payer, borrower, repayAmount);
        if (allowed != 0) {
            revert RepayBorrowGuptrollerRejection(allowed);
        }
    ...
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, GTokenInterface gTokenCollateral) internal {
        ...
        uint allowed = guptroller.liquidateBorrowAllowed(address(this), address(gTokenCollateral), liquidator, borrower, repayAmount);
        if (allowed != 0) {
            revert LiquidateGuptrollerRejection(allowed);
        }
        ...
        (uint amountSeizeError, uint seizeTokens) = guptroller.liquidateCalculateSeizeTokens(address(this), address(gTokenCollateral), actualRepayAmount);
    ...
    function seizeInternal(address seizerToken, address liquidator, address borrower, uint seizeTokens) internal {
        ...
        uint allowed = guptroller.seizeAllowed(address(this), seizerToken, liquidator, borrower, seizeTokens);
        if (allowed != 0) {
            revert LiquidateSeizeGuptrollerRejection(allowed);
        }
    ...
    /**
      * @notice Sets a new guptroller for the market
      * @dev Admin function to set a new guptroller
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function _setGuptroller(GuptrollerInterface newGuptroller) override public returns (uint) {
        // Check caller is admin
        if (msg.sender != admin) {
            revert SetGuptrollerOwnerCheck();
        }

        GuptrollerInterface oldGuptroller = guptroller;
        // Ensure invoke guptroller.isGuptroller() returns true
        require(newGuptroller.isGuptroller(), "marker method returned false");

        // Set market's guptroller to newGuptroller
        guptroller = newGuptroller;

        // Emit NewGuptroller(oldGuptroller, newGuptroller)
        emit NewGuptroller(oldGuptroller, newGuptroller);
